...
import { asOpenAIModel, getCloudCatalog, getUnifiedCatalog, pricingForCloudModel } from './router/catalog.js';
import { resolveModel } from './router/model-registry.js';
import { assertKeyPolicy, handleOpenAI, handlePiAnthropic, recordCloudAnthropic } from './router/request-handler.js';
...
async function resolveRequestedModel(requestedModel) {
    const direct = resolveModel(requestedModel, []);
    if (direct?.kind === 'pi') return direct;
    const cloudModels = await getCloudCatalog(accountManager);
    return resolveModel(requestedModel, cloudModels.map(model => model.id));
}
...
// Keeps the legacy administrator key while supporting generated, scoped router keys.
app.use('/v1', async (req, res, next) => {
    try {
        ...
        if (config.apiKey && providedKey === config.apiKey) {
            req.routerApiKey = { legacy: true, name: 'Legacy administrator key' };
            return next();
        }

        const generated = await routerApiKeyStore.authenticate(providedKey);
        if (generated) {
            req.routerApiKey = generated;
            return next();
        }

        const hasGeneratedKeys = (await routerApiKeyStore.list()).length > 0;
        if (!config.apiKey && !hasGeneratedKeys) {
            req.routerApiKey = { unprotected: true, name: 'Unprotected compatibility mode' };
            return next();
        }
        ...
        return res.status(500).json({ type: 'error', error: { type: 'api_error', message: 'API key validation failed' } });
    }
});
...
app.get('/v1/models', async (req, res) => {
    try {
        await ensureInitialized();
        let models = await getUnifiedCatalog(accountManager, { includeUnavailable: false });
        const key = req.routerApiKey;
        if (key && !key.legacy && !key.unprotected) {
            models = models.filter(model => routerApiKeyStore.allows(
                key,
                model.id,
                model.provider === 'cloudcode' ? `cloudcode/${model.id}` : model.id,
                model.provider
            ));
        }
        res.json({ object: 'list', data: models.map(asOpenAIModel) });
    } catch (error) {
...
app.post('/v1/chat/completions', async (req, res) => {
    try {
        ...
        if (!Array.isArray(req.body?.messages)) {
            return res.status(400).json({ error: { type: 'invalid_request_error', message: 'messages is required and must be an array' } });
        }
        const requestedModel = req.body.model;
        if (!requestedModel) {
            return res.status(400).json({ error: { type: 'invalid_request_error', message: 'model is required' } });
        }
        const resolved = await resolveRequestedModel(requestedModel);
        if (!resolved) {
            return res.status(400).json({ error: { type: 'invalid_request_error', message: `Unknown model: ${requestedModel}. Use /v1/models to list available models.` } });
        }
        assertKeyPolicy(req, resolved);
        await handleOpenAI(req, res, resolved, accountManager, FALLBACK_ENABLED, pricingForCloudModel(resolved.id));
    } catch (error) {
        logger.error('[OpenAI API] Error:', error);
        if (res.headersSent) return res.end();
        const status = error.statusCode || (error.message?.includes('rate') ? 429 : 500);
        res.status(status).json({ error: { type: status === 401 ? 'authentication_error' : 'api_error', message: error.message } });
    }
});
...
app.post('/v1/messages', async (req, res) => {
    try {
        ...
        if (!resolved) {
            return res.status(400).json({
                type: 'error',
                error: { type: 'invalid_request_error', message: `Unknown model: ${requestedModel}. Use /v1/models to see available models.` }
            });
        }
        assertKeyPolicy(req, resolved);

        if (resolved.kind === 'pi') {
            return await handlePiAnthropic(req, res, resolved);
        }
        ...
        if (validationAccount) {
            ...
            if (!valid) throw new Error(`invalid_request_error: Invalid model: ${modelId}. Use /v1/models to see available models.`);
        }
        ...
        const cloudPricing = pricingForCloudModel(modelId);
        ...
        if (stream) {
            ...
            try {
                ...
                const captureUsage = event => {
                    if (event?.message?.usage) Object.assign(streamUsage, event.message.usage);
                    if (event?.usage) Object.assign(streamUsage, event.usage);
                };
                ...
                recordCloudAnthropic(req, request, { usage: streamUsage }, resolved, requestStarted, cloudPricing);
                res.end();
            ...
            recordCloudAnthropic(req, request, response, resolved, requestStarted, cloudPricing);
            res.json(response);
        }
        ...
        if (errorType === 'authentication_error' && !error.statusCode) {
