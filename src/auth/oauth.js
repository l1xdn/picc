...
export function assertGoogleOAuthConfigured() {
    ...
    if (missing.length) {
        throw new Error(`Google OAuth is not configured. Set ${missing.join(' and ')} in ~/.config/antigravity-proxy/.env or the process environment.`);
    }
}

async function safeOAuthError(response, operation) {
    try {
        const body = await response.json();
        const message = body?.error_description || body?.error?.message || body?.error;
        if (typeof message === 'string' && message.length <= 500) return `${operation}: ${message}`;
    } catch {
        // Never include an unstructured provider body; it may contain credentials.
    }
    return `${operation}: provider returned HTTP ${response.status}`;
}
...
export async function exchangeCode(code, verifier) {
    ...
    if (!response.ok) {
        const error = await safeOAuthError(response, 'Token exchange failed');
    ...
    if (!tokens.access_token) {
        logger.error('[OAuth] Token exchange response did not contain an access token');
...
export async function refreshAccessToken(compositeRefresh) {
    ...
    if (!response.ok) {
        throw new Error(await safeOAuthError(response, 'Token refresh failed'));
    }
