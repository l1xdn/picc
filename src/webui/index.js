...
function createAuthMiddleware() {
    return (req, res, next) => {
        ...
        if (isApiRoute) {
            // Account, quota, and credential responses must never enter browser or proxy caches.
        ...
        const isProtected = (isApiRoute && !isConfigGet) || req.path === '/account-limits' || req.path === '/health';

        if (isProtected) {
            // Passwords in query strings leak through history, logs, and referrers;
            // accept the dedicated header only.
...
export function mountWebUI(app, dirname, accountManager) {
    ...
    // Unified router provider, API key, Pi import, and expense APIs
    mountRouterAdmin(app, accountManager);
    ...
    app.get('/api/logs/stream', (req, res) => {
        ...
        const sendLog = (log) => {
            if (!res.destroyed && !res.writableEnded) res.write(`data: ${JSON.stringify(log)}\n\n`);
        };
        ...
        req.once('close', () => {
            ...
            if (logger.off) logger.off('log', sendLog);
        });
    });
