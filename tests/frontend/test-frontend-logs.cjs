...
const WEBUI_PASSWORD = process.env.TEST_WEBUI_PASSWORD || process.env.WEBUI_PASSWORD || '';
...
function request(path, options = {}) {
    return new Promise((resolve, reject) => {
        ...
        const req = http.request(url, {
            ...
            headers: { ...(WEBUI_PASSWORD ? { 'x-webui-password': WEBUI_PASSWORD } : {}), ...(options.headers || {}) }
        }, (res) => {
            let data = '';
    ...
    },
    {
        ...
        async run() {
            return new Promise((resolve, reject) => {
                ...
                const req = http.request(url, {
                    headers: WEBUI_PASSWORD ? { 'x-webui-password': WEBUI_PASSWORD } : {}
                }, (res) => {
