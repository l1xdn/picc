...
document.addEventListener('alpine:init', () => {
    Alpine.store('global', {
        init() {
            // Hash-based routing
            const validTabs = ['dashboard', 'models', 'accounts', 'imported-keys', 'api-keys', 'expenses', 'logs', 'settings'];
            const validSettingsTabs = ['ui', 'claude', 'pi', 'models', 'server'];
            ...
            let { tab: initialTab, subtab: initialSubtab } = parseHash(getHash());
            // The former Router page mixed upstream credentials with inbound keys.
            // Keep old bookmarks useful by sending them to the dedicated API Keys page.
            if (initialTab === 'router') initialTab = 'api-keys';
