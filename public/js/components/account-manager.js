...
window.Components.accountManager = () => ({
    ...
    providerAccountsHandler: null,
    selectedProviderQuota: null,
    selectedProviderQuotaLabel: '',
    ...
    init() {
        this.fetchProviderAccounts();
        this.providerAccountsHandler = () => this.fetchProviderAccounts();
        window.addEventListener('provider-accounts-changed', this.providerAccountsHandler);
    ...
    destroy() {
        if (this.providerAccountsHandler) {
            window.removeEventListener('provider-accounts-changed', this.providerAccountsHandler);
        }
    },
    ...
    async routerRequest(url, options = {}) {
        ...
        if (!result.response.ok || data.status === 'error') {
            throw new Error(data.error || `HTTP ${result.response.status}`);
        }
        return data;
    },

    async fetchProviderAccounts(forceQuota = false) {
        ...
        try {
            ...
            this.fetchProviderQuotas(forceQuota);
        } catch (error) {
            Alpine.store('global').showToast(`Could not load provider accounts: ${error.message}`, 'error');
        } finally {
            this.providerAccountsLoading = false;
        }
    },

    async fetchProviderQuotas(force = false) {
        try {
            const data = await this.routerRequest(`/api/router/quotas${force ? '?refresh=true' : ''}`);
            for (const providerQuota of data.providers || []) {
                const provider = this.providerStatuses.find(item => item.id === providerQuota.id);
                if (!provider) continue;
                for (const accountQuota of providerQuota.accounts || []) {
                    const account = (provider.accounts || []).find(item => item.id === accountQuota.id);
                    if (!account) continue;
                    account.quota = accountQuota.quota;
                    if (accountQuota.quota?.tier) account.tier = accountQuota.quota.tier;
                    if (accountQuota.quota?.identity) account.identity = accountQuota.quota.identity;
                }
            }
        } catch (error) {
    ...
    get providerAccountGroups() {
        const query = this.searchQuery.toLowerCase().trim();
        const order = ['openai-codex', 'anthropic', 'github-copilot'];
        return this.providerStatuses.filter(provider => provider.supportsOAuth).map(provider => ({
            ...provider,
            accounts: (provider.accounts || []).filter(account => {
                if (account.type !== 'oauth') return false;
                return !query || `${provider.name} ${provider.id} ${account.label || ''}`.toLowerCase().includes(query);
            })
        })).filter(provider => !query || provider.accounts.length > 0 || `${provider.name} ${provider.id}`.toLowerCase().includes(query))
            .sort((a, b) => (order.indexOf(a.id) < 0 ? 99 : order.indexOf(a.id)) - (order.indexOf(b.id) < 0 ? 99 : order.indexOf(b.id)));
    },

    get oauthAccountCount() {
        return this.providerStatuses.reduce((total, provider) =>
            total + (provider.accounts || []).filter(account => account.type === 'oauth').length, 0);
    },

    providerAccountDisplay(provider, account) {
        const value = account.identity || account.label || `${provider.name} account`;
        if (value.includes('@')) return this.formatEmail(Redact.email(value));
        if (value.length > 42) return `${value.slice(0, 18)}…${value.slice(-10)}`;
        return value;
    },

    providerAccountTier(account) {
        return account.tier ? String(account.tier).replaceAll('_', ' ').toUpperCase() : 'NOT REPORTED';
    },

    providerQuotaDetail(quota) {
        if (!quota) return '';
        const active = quota.windows?.find(item => item.remainingFraction === quota.remainingFraction);
        const parts = active?.name ? [active.name] : [];
        if (quota.resetAt) parts.push(`resets ${window.utils.formatTimeUntil(quota.resetAt)}`);
        return parts.join(' · ');
    },

    openProviderQuota(provider, account) {
        if (account.quota?.status !== 'reported') return;
        this.selectedProviderQuota = account.quota;
        this.selectedProviderQuotaLabel = `${provider.name} · ${this.providerAccountDisplay(provider, account)}`;
        document.getElementById('provider_quota_modal')?.showModal();
    },

    async toggleProviderAccount(provider, account, enabled) {
        const previous = account.enabled;
        account.enabled = enabled;
        try {
            await this.routerRequest(`/api/router/providers/${encodeURIComponent(provider.id)}/accounts/${encodeURIComponent(account.id)}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ enabled })
            });
            Alpine.store('global').showToast(`${provider.name} account ${enabled ? 'enabled' : 'disabled'}`, 'success');
        } catch (error) {
    ...
    async deleteProviderAccount(provider, account) {
        if (!confirm(`Remove “${account.label || provider.name}” from ${provider.name}?`)) return;
        try {
            await this.routerRequest(`/api/router/providers/${encodeURIComponent(provider.id)}/accounts/${encodeURIComponent(account.id)}`, { method: 'DELETE' });
            Alpine.store('global').showToast(`${provider.name} account removed`, 'success');
