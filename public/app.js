...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('routerManager', window.Components.routerManager);
    Alpine.data('expenses', window.Components.expenses);
    Alpine.data('piConfig', window.Components.piConfig);
    Alpine.data('addAccountModal', window.Components.addAccountModal);

    // View Loader Directive
    Alpine.directive('load-view', (el, { expression }, { evaluate, cleanup }) => {
        ...
        const initializeView = (html) => {
            // Prevent Alpine's MutationObserver and this explicit initialization from
            // both mounting the same component tree (which previously duplicated
            // EventSources, watchers, charts, and retained view state).
            Alpine.mutateDom(() => {
                el.innerHTML = html;
        ...
        if (window.viewCache.has(viewName)) {
            initializeView(window.viewCache.get(viewName));
            return;
        }

        fetch(`views/${viewName}.html?t=${Date.now()}`, { signal: controller.signal })
            .then(response => {
